...
public:
    void on_class_begin(const clang::CXXRecordDecl* rec, const GenContext& gc) override {
        if (!component_ops_detail::derives_from_component(rec))
            return;
        *gc.os << "#include \"scene2/components/component_ops.h\"\n";
    }
    ...
    void on_class_end(const clang::CXXRecordDecl* rec, const GenContext& gc) override {
        ...
        *gc.os << "            return scene.get_entities()[entity::index(id)].mask.test(type_id<"<< gc.type <<">());\n";
        *gc.os << "        },\n";
